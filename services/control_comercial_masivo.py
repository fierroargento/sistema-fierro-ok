...
def construir_bandeja(simulaciones, items):
    filas = []
    for simulacion in simulaciones:
        ...
        filas.append(evaluar_control(
            simulacion,
            item.precio_final_centavos if item else None,
            "lista_interna" if item else None,
        ))
...
def exportar_bandeja_excel(filas):
    ...
    hoja.append(["SKU", "LISTA_CANAL", "ESTADO", "FUENTE_PRECIO", "PRECIO_ACTUAL", "PRECIO_MINIMO", "PRECIO_PROPUESTO", "LIQUIDACION_ACTUAL", "PISO_MINIMO", "DIFERENCIA", "DIFERENCIA_PCT", "CAMBIA_CARGO", "CAMBIA_ENVIO"])
    for fila in filas:
        actual = fila["actual"]
        hoja.append([
            ...
            fila["diferencia_pct"], "SI" if fila["cambia_cargo"] else "NO",
