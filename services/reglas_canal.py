...
def crear_regla_canal(lista, *, nombre, comision_pct, publicidad_pct=0, financiacion_pct=0, devoluciones_pct=0, umbral_envio_centavos, costo_envio_default_centavos, incremento_redondeo_centavos, observacion, usuario, ReglaCanalVersion, db_session):
    ...
    try:
        ...
        devoluciones = Decimal(str(devoluciones_pct or 0).replace(",", "."))
    except InvalidOperation as error: raise ValueError("Publicidad o financiacion no son validas.") from error
    if publicidad < 0 or financiacion < 0 or devoluciones < 0 or comision + publicidad + financiacion + devoluciones >= 100:
        raise ValueError("Los costos porcentuales deben ser no negativos y sumar menos de 100%.")
