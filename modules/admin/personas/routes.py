...
def crear_blueprint_personas(*,dependencias):
 ...
 def datos(o,u):
  empleados=m["EmpleadoProductivo"].query.filter_by(organizacion_id=o.id,unidad_negocio_id=u.id,tipo_registro="empleado").order_by(m["EmpleadoProductivo"].nombre.asc()).all();turnos=m["TurnoLaboralPlanificado"].query.filter_by(organizacion_id=o.id,unidad_negocio_id=u.id).order_by(m["TurnoLaboralPlanificado"].inicio.asc()).all();novedades=m["NovedadLaboralPreparatoria"].query.filter_by(organizacion_id=o.id,unidad_negocio_id=u.id).order_by(m["NovedadLaboralPreparatoria"].fecha_desde.asc()).all();return empleados,turnos,novedades
 @bp.route("/admin/personas")
 @dependencias["login_required"]
 def panel():
  _x,o,u,r=acceso()
  if r is not None:return r
  e,t,n=datos(o,u);control=controlar_personas(organizacion_id=o.id,unidad_negocio_id=u.id,empleados=e,turnos=t,novedades=n);return render_template("admin_personas.html",organizacion=o,unidad_activa=u,empleados=e,turnos=t,novedades=n,control=control,ok_feedback=request.args.get("ok"),error=request.args.get("error"))
 ...
 def control():
  _x,o,u,r=acceso()
  if r is not None:return r
  e,t,n=datos(o,u);resultado=controlar_personas(organizacion_id=o.id,unidad_negocio_id=u.id,empleados=e,turnos=t,novedades=n);return send_file(exportar_control(resultado),as_attachment=True,download_name="control_personas_preparatorio.json",mimetype="application/json")
 return bp
