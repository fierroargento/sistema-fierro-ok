 ...
 def guardar():
  ...
  try:
   ...
   if accion=="crear_caso":
    ...
    caso=m["CasoPostventa"].query.filter_by(id=int(request.form.get("caso_id")),organizacion_id=o.id,unidad_negocio_id=u.id).first();obj=proponer_resolucion(request.form,caso=caso,pedido=caso.pedido if caso else None,organizacion_id=o.id,unidad_negocio_id=u.id,Propuesta=m["PropuestaResolucionPostventa"],db_session=db.session);mensaje=f"Resolucion {obj.id} propuesta sin ejecutar."
   elif accion=="estado":
    caso=m["CasoPostventa"].query.filter_by(id=int(request.form.get("caso_id")),organizacion_id=o.id,unidad_negocio_id=u.id).first();obj=cambiar_estado(caso,pedido=caso.pedido if caso else None,organizacion_id=o.id,unidad_negocio_id=u.id,estado=request.form.get("estado"),db_session=db.session);mensaje=f"Caso {obj.id} actualizado a {obj.estado}."
   elif accion=="agregar_item":
    caso=m["CasoPostventa"].query.filter_by(id=int(request.form.get("caso_id")),organizacion_id=o.id,unidad_negocio_id=u.id).first();producto=m["Producto"].query.filter_by(id=int(request.form.get("producto_id")),organizacion_id=o.id).first();obj=agregar_item(request.form,caso=caso,pedido=caso.pedido if caso else None,producto=producto,organizacion_id=o.id,unidad_negocio_id=u.id,Item=m["ItemCasoPostventa"],db_session=db.session);mensaje=f"Item {obj.id} documentado sin recepcion ni stock."
   elif accion=="agregar_evidencia":
    caso=m["CasoPostventa"].query.filter_by(id=int(request.form.get("caso_id")),organizacion_id=o.id,unidad_negocio_id=u.id).first();obj=agregar_evidencia(request.form,caso=caso,pedido=caso.pedido if caso else None,organizacion_id=o.id,unidad_negocio_id=u.id,Evidencia=m["EvidenciaCasoPostventa"],db_session=db.session);mensaje=f"Evidencia {obj.id} registrada solo como referencia interna."
