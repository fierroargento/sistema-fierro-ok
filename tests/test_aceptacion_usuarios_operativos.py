...
import pytest
from services.aceptacion_usuarios_operativos import CASOS,PRUEBAS_DATOS,evaluar,exportar,plantilla
...
def resultados(resultado="aprobado"):
 d=json.loads(plantilla(organizacion_id=7,unidad_negocio_id=3).read());d["participantes"]=["Martín","Ezequiel","Franco"];[(x.update(resultado=resultado,responsable="Usuario de prueba",observacion="Validación documentada")) for x in d["casos"]];[(x.update(resultado=resultado,responsable="Usuario de prueba",evidencia=f"captura-{x['codigo']}.png",observacion="Validación documentada")) for x in d["pruebas_datos"]];return io.BytesIO(json.dumps(d).encode())
def test_aprueba_nueve_areas_y_firma_acta_sin_habilitar():
 r=evaluar(preparacion(),resultados(),organizacion_id=7,unidad_negocio_id=3);assert r["estado"]=="uat_aprobada_para_revision_humana" and r["resumen"]["casos_aprobados"]==len(CASOS) and r["resumen"]["pruebas_datos_aprobadas"]==len(PRUEBAS_DATOS) and len(r["huella_acta_uat"])==64;assert r["corte_dux_autorizado"] is False and r["integraciones_autorizadas"] is False and not any(r["controles"].values())
...
def test_exige_recorrido_datos_evidencia_y_no_duplica_pruebas():
 d=json.loads(resultados().read());d["pruebas_datos"][0]["evidencia"]="";d["pruebas_datos"][1]=dict(d["pruebas_datos"][0]);r=evaluar(preparacion(),io.BytesIO(json.dumps(d).encode()),organizacion_id=7,unidad_negocio_id=3);c={x["codigo"] for x in r["hallazgos"]};assert {"evidencia_prueba_datos_faltante","prueba_datos_duplicada","prueba_datos_faltante"}<=c
...
def test_ruta_panel_y_plantilla_disponibles():
 r=Path("modules/admin/estructura/routes.py").read_text(encoding="utf-8");h=Path("templates/admin_aceptacion_usuarios_operativos.html").read_text(encoding="utf-8");assert "evaluar_uat(" in r and "plantilla_uat(" in r and "resolver_acceso()" in r;assert "nueve áreas" in h and "Recorrido visual de datos sintéticos" in h
